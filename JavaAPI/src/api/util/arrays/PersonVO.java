...
public class PersonVO implements Comparable<PersonVO>{
	...
	//속성을 전부 매개변수로 받는 생성자 - 인스턴스 생성 후 값을 빠르게 대입하기 위해 생성
	...
	//d일반적으로 VO 클래스의 경우는 setter를 만들지 않음
	public int getNum() {
		return num;
	}
	...
	//출력하는 메서드에 인스턴스 이름을 대입하면 이 메서드가 호출되어서 출력된다.
	public String toString() {
		return "PerconVO [num=" + num + ", name=" + name + ", age=" + age + "]";
	}
	...
	public int compareTo(PersonVO o) {
		//크기 비교 방법 설정
		return o.name.compareTo(this.name); //음수를 리턴하면 내림차순.
	}
